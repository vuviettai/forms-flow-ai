...
RUN chmod +x /usr/share/nginx/html/root-config/config/env.sh

# Start Nginx server with environment setup
#CMD ["/bin/bash", "-c", "nginx -g \"daemon off;\""]
CMD ["/bin/bash", "-c", "/usr/share/nginx/html/root-config/config/env.sh && nginx -g 'daemon off;'"]